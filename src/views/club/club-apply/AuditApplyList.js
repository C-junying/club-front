...
// 审核社团的操作
function AuditApplyList() {
  // store
  const { clubProcessStore } = useRootStore();
  // 通知
  const [messageApi, contextHolder] = message.useMessage();
  useEffect(() => {
    clubProcessStore.getApplyClubList();
  }, []);
  const columns = [
    {
      title: '社团名称',
  ...
  const handleAudit = () => {
    auditForm
      ...
      .then((values) => {
        ...
        clubProcessStore.auditClub(values).then((res) => {
          setApplyState(0);
          setAuditOpen(false);
  ...
  const onSearch = (value) => {
    if (value === '') {
      clubProcessStore.getApplyClubList(true);
    } else {
      clubProcessStore.getSearch(value);
    }
  };
  return (
    <div>
      ...
      <Table
        ...
        pagination={{
          ...
          defaultPageSize: 5,
          total: clubProcessStore.applyClubList.length,
        ...
        dataSource={clubProcessStore.applyClubList}
      />
      <Modal
        open={auditOpen}
        title="审核申请社团"
...
export default observer(AuditApplyList);
